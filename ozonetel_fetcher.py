...
def compute_stats(records):
    """Return per-agent dict with call counts, talk time, drops etc."""
    agents = defaultdict(lambda: {
        "total_calls": 0,
        "answered": 0,
        "unanswered": 0,
        "dropped": 0,         # answered but talktime < 5s
        "talk_seconds": 0,
    ...
    for r in records:
        agent = (r.get("agentConnected") or "").strip() or "Unknown"
        ...
        if status == "answered":
            ...
            if talk_sec < 5:
                a["dropped"]  += 1   # answered but immediately dropped
        else:
            a["unanswered"]   += 1
    ...
    result = {}
    for agent, d in sorted(agents.items(), key=lambda x: -x[1]["total_calls"]):
        result[agent] = {
            **d,
            "locations": ", ".join(sorted(d["locations"] - {""})) or "—",
            ...
            "avg_talk_sec": round(d["talk_seconds"] / d["answered"]) if d["answered"] else 0,
        }
    return result
