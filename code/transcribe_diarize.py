...
import os, sys, json, warnings, shutil, argparse
import numpy as np
...
def process_file(audio_path: str, whisper_model, pyannote_pipeline,
    ...
    try:
        ...
        # Save outputs
        txt_path, json_path = save_outputs(turns, t_info, audio_path, output_dir=output_dir)
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe + diarize audio files")
    parser.add_argument("--date", default=None,
                        help="Process recording/YYYY-MM-DD/ → transcripts/YYYY-MM-DD/ "
                             "(default: flat recording/ → transcripts/)")
    args = parser.parse_args()

    # Resolve directories based on --date
    if args.date:
        input_dir  = os.path.join(BASE_DIR, "recording",    args.date)
        output_dir = os.path.join(BASE_DIR, "transcripts",  args.date)
        done_dir   = os.path.join(input_dir, "done")
    else:
        input_dir  = os.path.join(BASE_DIR, "recording")
        output_dir = os.path.join(BASE_DIR, "transcripts")
        done_dir   = os.path.join(BASE_DIR, "recording", "done")

    os.makedirs(done_dir,   exist_ok=True)
    os.makedirs(output_dir, exist_ok=True)

    # Collect audio files — skip ones already transcribed (idempotent resume)
    audio_files = sorted(
        p for p in Path(input_dir).iterdir()
        if p.is_file()
        and p.suffix.lower() in AUDIO_EXTS
        and not p.name.endswith("_16k.wav")
    )
    ...
    for p in audio_files:
        stem      = p.stem
        json_out  = Path(output_dir) / f"{stem}_diarized.json"
        if json_out.exists():
            skipped += 1
        else:
            pending.append(p)
    if skipped:
        print(f"Skipping {skipped} already-transcribed file(s) — transcript JSON exists.")
    audio_files = pending
    ...
    for i, audio_path in enumerate(audio_files, 1):
        ...
        # Move audio to done/ regardless of outcome (resumable if restarted)
        ...
        print(f"  ↳ Moved audio → done/", flush=True)
