...
def resolve_agent_by_fullname(full_name: str) -> dict:
    """
    Reverse lookup by full name (as returned in new Ozonetel filename format).
    e.g. "Anamika_Kumar" or "Anamika Kumar" → returns agent dict.

    Tries exact match first, then case-insensitive, then first-name-only fallback.
    """
    # Normalise: replace underscores with spaces, strip
    name = full_name.replace("_", " ").strip()

    # Build reverse map on first call (all agents combined)
    for lookup in (BD_SALES_AGENTS, SUPPORT_AGENTS):
        for key, info in lookup.items():
            if info["full_name"].lower() == name.lower():
                return info

    # First-name-only fallback (e.g. "Anamika" → "Anamika Kumar")
    first = name.split()[0].lower()
    for lookup in (BD_SALES_AGENTS, SUPPORT_AGENTS):
        for key, info in lookup.items():
            if info["full_name"].lower().startswith(first):
                return info

    # Last resort — return what we can infer
    return {
        "full_name": name,
        "location":  "Unknown",
        "team":      "Unknown",
